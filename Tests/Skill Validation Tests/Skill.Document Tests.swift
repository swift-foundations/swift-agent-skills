...
extension Skill {
    ...
    struct Test {
        ...
        struct Integration {
            @Test
            func `validates a full skill document against its directory name`() throws {
                let source = """
                    ---
                    name: example-skill
                    description: |
                      Validates the canonical example.
                      Apply when the example is relevant.
                    ---

                    # Example

                    Body prose with a rule.
                    """
                ...
                #expect(document.name == "example-skill")
                #expect(document.body.contains("Body prose with a rule."))
            }
        }
    }
}
